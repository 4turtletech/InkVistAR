...
- **Client-Side Validation (Layer 1 — UX):** Every input field, dropdown, date picker, textarea, and any other form element that accepts user input MUST include:
  ...
  - **Visual feedback:** Invalid fields must display clear, inline error messages (red border + helper text). Do NOT use browser `alert()` or React Native `Alert.alert()` for form validation or routine success feedback. Reserve themed confirmation modals for destructive or consequential choices.
  - **Mobile alert rule:** Do not introduce native `Alert.alert()` UI. Show field and submission failures inline, handle routine success through the existing screen flow or a non-blocking themed notice, and use an in-app themed modal when confirmation is genuinely required.
