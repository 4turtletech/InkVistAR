...
- **Temporary Mobile CAPTCHA Bypass:** `BYPASS_MOBILE_REGISTRATION_CAPTCHA=true` skips CAPTCHA only for requests carrying the mobile-app client header. Web registration continues to require real CAPTCHA. This header is not proof of a genuine app and can be copied by an attacker, so keep the switch temporary and return it to `false` after replacing the native CAPTCHA flow. Email OTP verification remains required.
