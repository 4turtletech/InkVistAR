...
function ArtistSideNav() {
    ...
    const artistUser = JSON.parse(localStorage.getItem('user') || 'null');
    const artistId = artistUser ? artistUser.id : null;
    ...
    return (
        ...
        <aside className={`artist-sidenav ${collapsed ? 'collapsed' : ''} ${mobileOpen ? 'mobile-open' : ''}`}>
