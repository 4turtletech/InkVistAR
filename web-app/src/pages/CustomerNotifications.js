                    ...
                    <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '500px', width: '90%', padding: '30px', borderRadius: '16px', background: '#fff', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #e2e8f0', paddingTop: '15px' }}>
                            ...
                            <div className="notif-actions" style={{ display: 'flex', gap: '10px' }}>
                                {selectedNotification.type === 'appointment_reminder' && <a href="/customer/bookings" className="btn btn-primary" style={{ textDecoration: 'none' }}>View Upcoming</a>}
                                {selectedNotification.type === 'pos_invoice' && <a href={`${API_URL}/api/invoices/${selectedNotification.related_id}`} target="_blank" rel="noopener noreferrer" className="btn btn-primary" style={{ textDecoration: 'none' }}>View Invoice</a>}
                                {selectedNotification.type === 'aftercare_reminder' && <a href="/customer/aftercare" className="btn btn-primary" style={{ textDecoration: 'none', backgroundColor: '#06b6d4', border: 'none' }}>View Guide</a>}
                                {selectedNotification.type === 'review_prompt' && <a href={`/customer/reviews/new?appointment=${selectedNotification.related_id}`} className="btn btn-primary" style={{ textDecoration: 'none', backgroundColor: '#f59e0b', border: 'none' }}>Leave Review</a>}
