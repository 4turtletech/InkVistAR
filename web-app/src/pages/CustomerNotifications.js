...
function CustomerNotifications() {
    ...
    const getNotificationInvoiceNumber = (notification) => {
        const messageMatch = notification?.message?.match(/INV-\d+/);
        if (messageMatch) return messageMatch[0];
        const legacyReference = String(notification?.related_id || '');
        return /^INV-\d+$/.test(legacyReference) ? legacyReference : null;
    };

    const openInvoice = (invoiceNumber) => {
        if (!invoiceNumber) return;
        navigate(`/customer/invoice/${encodeURIComponent(invoiceNumber)}`);
    };
                                                    ...
                                                    <div className="notif-main" style={{ display: 'flex', alignItems: 'center', gap: '20px', overflow: 'hidden' }}>
                                                        ...
                                                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px' }}>
                                                            ...
                                                            <div className="notif-actions" style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                                                                ...
                                                                {['pos_invoice', 'invoice_pending', 'invoice_paid'].includes(n.type) && (
                    ...
                    <div className="modal-content" onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{ padding: '16px 24px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                                {selectedNotification.related_id && !['pos_invoice', 'invoice_pending', 'invoice_paid', 'review_prompt', 'aftercare_reminder', 'aftercare_daily', 'email_change', 'password_change', 'payment_success'].includes(selectedNotification.type) && (
